...
public class WorkflowTransitionService extends BaseService<WorkflowTransition> {
	...
	public List<WorkflowTransition> getStartingWorkflowTransitions(EntityManager em, Workflow workflow) {
		TypedQuery<WorkflowTransition> query = em.createQuery(
				"SELECT wt FROM " + WorkflowTransition.class.getSimpleName() + " wt WHERE wt.workflow.id= " + workflow.getId() + " AND wt.isFirstStep=true",
