...
public class TicketService extends BaseService<Ticket> {
	...
	public Ticket getTicketBySpaceAndNumber(long spaceId, String ticketNumber) {
		List<Ticket> resultList = getEntityManager().createQuery("SELECT t FROM " + Ticket.class.getSimpleName()
				+ " t WHERE t.space.id = '" + spaceId + "' AND t.ticketNumber = '" + ticketNumber + "'", Ticket.class)
				.getResultList();
