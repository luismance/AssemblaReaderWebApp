...
public class SpaceRestService {
	...
	public Response getSpaces(@HeaderParam("Authorization") String authorization) {
		...
		String spacesXml = RESTServiceUtil.sendGET("https://api.assembla.com/v1/spaces.xml", true, "Bearer " + valResult.getUser().getBearerToken());

		if (spacesXml.equals("401")) {

			RESTServiceUtil.refreshBearerToken(valResult.getUser());

			spacesXml = RESTServiceUtil.sendGET("https://api.assembla.com/v1/spaces.xml", true, "Bearer " + valResult.getUser().getBearerToken());
